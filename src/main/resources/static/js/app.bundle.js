	...
	app.listFilter = function (list, input, anchor) {
		...
		$(input).change( function () {
			...
			if(filter.length > 1) {
				...
				if (listPrev){	
					listPrev.text("총 " + $(list).find('[data-filter-tags]').length + "개 메뉴 중 " + $(list).find('li.js-filter-show').length + "개 조회");
				}
				...
				switch ( true ) {
					...
					case ( actiontype === 'panel-close' ):
						...
						if (typeof bootbox  != 'undefined') {
							...
							bootbox.confirm({
								title: "<i class='" + myapp_config.appIconPrefix +  " fa-times-circle text-danger mr-2'></i><span class='fw-500'>&nbsp;'" +selectedPanel.children('.panel-hdr').children('h2').text().trim()+ "'&nbsp;</span> 패널을 닫으시겠습니까?",
								message: "<span><strong>주의:</strong> 되돌릴 수 없음을 기억하세요!</span>",
								centerVertical: true,
								swapButtonOrder: true,
								buttons: {
									confirm: {
							...
							if (confirm(selectedPanel.children('.panel-hdr').children('h2').text().trim() + ' 패널을 닫으시겠습니까?' )) {
								killPanel();
							}
...
var layouts = (function(setlayout) {
	...
	console.log('setlayout.theme');
	setlayout.theme = function(themename,save) {
		console.log('setlayout.theme2');
